  ...
  if R.height > 18 then
    local function today()
      ...
      local function suffix()
        if day:match('1$') then
          return 'st'
        elseif day:match('2$') then
          return 'nd'
        elseif day:match('3$') then
          return 'rd'
        else
          return 'th'
        end
          ...
          callback = function()
            ...
            if state.show_image then
              vim.defer_fn(function()
                ...
                chafa_cmd = chafa_cmd .. ' ~/.config/nvim/lua/raf/dashboard/moon.jpg' .. redirect

                pcall(os.execute, [[printf '[3H']] .. redirect)
                pcall(os.execute, chafa_cmd)
                require("dashboard.puns").print(state.width, redirect)
                pcall(os.execute,
                  [[printf '[]] .. (cursor_pos[1] + 1) .. ';' .. (cursor_pos[2] - 1) .. [[H']] .. redirect)
              end, 25)
            end
          end
        })
